...
const ContentSchedulerModule = ({ onNavigateToCreate, scheduledReminders = [], onUpdateReminders }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Vista Calendario */}
      {viewMode === 'calendar' && (
        <div className={`rounded-2xl border p-6 ${theme === 'dark' ? 'bg-[#0b0c10]/50 border-gray-800' : 'bg-white border-gray-200 shadow-sm'}`}>
          {/* Navegación del calendario */}
          <div className="flex items-center justify-between mb-8">
            <h3 className={`text-xl font-bold capitalize ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
              {selectedDate.toLocaleDateString('es-ES', { month: 'long', year: 'numeric' })}
            </h3>
            <div className={`flex items-center gap-2 rounded-lg p-1 border ${theme === 'dark' ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}>
              <button
                onClick={() => setSelectedDate(new Date(selectedDate.getFullYear(), selectedDate.getMonth() - 1))}
                className={`p-2 rounded-md transition-colors ${theme === 'dark' ? 'hover:bg-gray-700 text-gray-400' : 'hover:bg-white hover:shadow-sm text-gray-600'}`}
              >
                <ChevronLeft className="w-5 h-5" />
              </button>
              <button
                onClick={() => setSelectedDate(new Date())}
                className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${theme === 'dark'
          ...
          <div className="grid grid-cols-7 gap-1 mb-4">
            {['Dom', 'Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb'].map(day => (
              <div key={day} className={`p-2 text-center text-xs font-semibold uppercase tracking-wider ${theme === 'dark' ? 'text-gray-500' : 'text-gray-400'}`}>
                {day}
              </div>
            ))}
          </div>
          ...
          {/* Calendario */}
          <div className={`grid grid-cols-7 gap-px rounded-lg overflow-hidden border ${theme === 'dark' ? 'bg-gray-800 border-gray-800' : 'bg-gray-200 border-gray-200'}`}>
            {generateCalendarDays().map((date, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`min-h-[120px] p-2 transition-colors relative group ${isCurrentMonth
                    ? (theme === 'dark' ? 'bg-[#0b0c10]' : 'bg-white')
                    : (theme === 'dark' ? 'bg-[#111216]' : 'bg-gray-50')
                    } ${isToday ? (theme === 'dark' ? 'bg-blue-900/10' : 'bg-blue-50') : ''}`}
                >
                  <div className="flex justify-between items-start mb-2">
                    <span className={`text-sm font-medium w-7 h-7 flex items-center justify-center rounded-full ${isToday
                      ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30'
                      : (isCurrentMonth ? (theme === 'dark' ? 'text-gray-300' : 'text-gray-700') : 'text-gray-500')
                      }`}>
                      {date.getDate()}
                    </span>
                    {/* Add Button on Hover - only for current/future dates */}
                    {date >= new Date(new Date().setHours(0, 0, 0, 0)) && (
                      <button
                        onClick={() => {
                          if (onNavigateToCreate) {
                            // Optionally pass date
                            onNavigateToCreate();
                          }
                        }}
                        className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-800 transition-all text-gray-400"
                      >
                  ...
                  <div className="space-y-1.5">
                    {postsForDay.slice(0, 3).map(post => (
                      <div
                        ...
                        className={`text-xs p-1.5 rounded cursor-pointer transition-all hover:scale-[1.02] border ${theme === 'dark'
                      ...
                      >
                        <div className="flex items-center gap-1.5 mb-1">
                          {(() => {
                            const LogoComponent = getPlatformLogo(post.platform);
                            return LogoComponent ? <LogoComponent size={12} /> : null;
                          })()}
                    ...
                    {postsForDay.length > 3 && (
                      <div className={`text-[10px] font-medium text-center py-1 rounded ${theme === 'dark' ? 'text-gray-500 hover:bg-gray-800' : 'text-gray-500 hover:bg-gray-100'
                        }`}>
      ...
      {/* Modal de Detalles - desactivado temporalmente para evitar errores de JSX */}
      {null}
    </div>
  );
};
